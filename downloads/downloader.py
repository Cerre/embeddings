...
class Downloader:
    ...
    async def download_audio(self, video):
        try:
            ...
            audio_streams.download(output_path=self.download_path, filename=f"{video.video_id}.mp4")
            return audio_file_path
        except PytubeError as e:
            print(f"An error occurred while downloading the video {video.title}: {e}")
            return None
        except Exception as e:
            print(f"An unexpected error occurred while processing the video {video.title}: {e}")
            return None
