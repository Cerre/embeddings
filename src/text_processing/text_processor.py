...
class TextProcessor:
    ...
    def remove_pagination_breaks(self, text: str) -> str:
        text = re.sub(
            r"-(\n)(?=[a-z])", "", text
        )  # Remove hyphens at the end of lines when the word continues on the next line
        text = re.sub(
            r"(?<=\w)(?<![.?!-]|\d)\n(?![\nA-Z])", " ", text
        )  # Replace line breaks that are not preceded by punctuation or list markers and not followed by an uppercase letter or another line break
        return text
    ...
    def save_processed_data(self, audio_file_name, sentences, metadata_dicts):
        ...
        print(
            f"Output files saved: {text_file_path}, {metadata_csv_path}, {metadata_json_path}"
        )
