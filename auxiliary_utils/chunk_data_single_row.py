...
def chunk_data(row, chunk_length_minutes=1, overlap_percentage=0.2):
    ...
    try:
        ...
        print(f"Failed to gather lines for videoId: {row['videoId']} with name {row['title']}")
        return []

    manuscript_index = next((i for i, line in enumerate(lines) if "Manuskript" in line), -1)
    ...
    for idx, line in enumerate(lines):
        ...
        if re.match(pattern, line):
            if current_text_accumulator:  # If there's accumulated text, append it before resetting
    ...
    try:
        assert len(valid_timestamps) == len(valid_texts), print(f"Length of timestamps and texts do not match for videoId: {row['videoId']} with name {row['title']}")
    except:
        breakpoint()
    while i < len(valid_timestamps):
        ...
        while i < len(valid_timestamps) and valid_timestamps[i] - chunk_start_time < chunk_length:
        ...
        if i < len(valid_timestamps):
            ...
            if end_of_current_chunk - 1 > 0:
                overlap_start_time = valid_timestamps[end_of_current_chunk - 1] - overlap_time
...
def process_row(row):
    ...
    chunk_dicts = [{'start_time': start_time, 'chunk_text': chunk_text} for start_time, chunk_text in chunks]
    return chunk_dicts
